...
public class DistributionHistogramDrawer {
	public static void evaluate(String path, double minCircleRadius, double maxCircleRadius, double delta,
		...
		try {
			...
			Map<Double, List<Double>> outputs = finder.execute(borders, calculator, minCircleRadius, maxCircleRadius,
					delta);
			System.out.println("Intersection distributions calculated.");
			
			File file = new File(String.format("/home/puya/Documents/FinalProject/Output/%s", dirName));
			PrintWriter writer = new PrintWriter(String.format("/home/puya/Documents/FinalProject/Output/%s", dirName), "UTF-8");
			writer.println(outputs.toString());
			writer.close();
			System.out.println("Intersection distributions saved to output file.");
